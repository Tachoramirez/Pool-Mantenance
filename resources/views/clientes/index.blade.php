<x-app-layout>
    <x-slot name="header">
        ...
        <div class="col-span-1"><h2 class="font-semibold text-xl text-gray-800 leading-tight">Clientes</h2></div>
        </div>
    ...
    <div class="py-4">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <x-primary-button class="mb-2">
                <a href="{{route('clientes.create')}}">{{__('Nuevo cliente')}}</a>
            </x-primary-button>
                    ...
                    <table class="w-full table-auto">
                        <thead class="bg-blue-50 text-xs font-semibold uppercase text-gray-400">
                            <tr>
                                ...
                                <th class="p-2">
                                    <div class="text-left font-semibold">Telefono</div>
                                </th>
                                <th class="p-2">
                                    <div class="text-left font-semibold">Correo</div>
                                </th>
                                <th class="p-2">
                                    <div class="text-left font-semibold">Vigencia</div>
                                </th>
                                <th class="p-2">
                        ...
                        <tbody class="divide-y divide-gray-100 text-sm">
                            ...
                            @foreach ($clientes as $cliente)
                                <tr>
                                    <td class="p-2"><div class="text-left font-medium">{{$cliente->name}}</div></td>
                                    <td class="p-2"><div class="text-left font-medium">{{$cliente->num_cel}}</div></td>
                                    <td class="p-2"><div class="text-left font-medium">{{$cliente->email}}</div></td>
                                    @if ($cliente->vigencia == 1)
                                        <td class="p-2"><div class="inline px-3 py-1 text-sm font-normal rounded-full text-emerald-500 gap-x-2 bg-emerald-100/100 dark:bg-gray-800">Activo</div></td>
                                    @else
                                        <td class="p-2"><div class="inline px-3 py-1 text-sm font-normal rounded-full text-emerald-500 gap-x-2 bg-red-100/100 dark:bg-gray-800">Inactivo</div></td>
                                    @endif
                                    <td class="p-2">
                                        <div class="flex justify-center">
                                            <a class="btn" href="{{route('clientes.show', $cliente->id)}}" title="ver">
                                            ...
                                            <a class="btn" href="{{route('clientes.edit', $cliente->id)}}" title="editar">
